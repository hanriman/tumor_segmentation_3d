...
def plot_low_data_efficiency():
    ...
    # Plot VisReg JEPA
    ax.plot(x, visreg_mean, marker='o', color=COLOR_VISREG, label='3D VisReg JEPA (Multiscale FPN)', zorder=5)
    ax.fill_between(x, visreg_mean - visreg_std, visreg_mean + visreg_std, color=COLOR_VISREG, alpha=0.18, zorder=4)
    
    # Plot nnU-Net
    ax.plot(x, nnunet_mean, marker='s', color=COLOR_NNUNET, label='3D nnU-Net Baseline (DynUNet)', zorder=3)
    ax.fill_between(x, nnunet_mean - nnunet_std, nnunet_mean + nnunet_std, color=COLOR_NNUNET, alpha=0.18, zorder=2)
    
    # Plot UNet
    ax.plot(x, unet_mean, marker='^', color=COLOR_UNET, label='3D Residual UNet Baseline', linestyle='--', zorder=3)
    ax.fill_between(x, unet_mean - unet_std, unet_mean + unet_std, color=COLOR_UNET, alpha=0.15, zorder=1)
    
    # Annotate 1% extreme label efficiency
    ax.annotate(r'$\mathbf{+30.22\%}$ absolute Dice' + '\n' + r'($1.79\times$ over nnU-Net; $2.8\times$ over UNet)', 
                xy=(0, 68.42), xytext=(0.4, 52.0),
                arrowprops=dict(facecolor=COLOR_VISREG, shrink=0.08, width=1.5, headwidth=6),
                fontsize=9.5, fontweight='bold', color=COLOR_VISREG,
...
def plot_ood_synthetic():
    ...
    ax1.bar(x - width, unet_dice, width, yerr=unet_err, capsize=4, label='3D Residual UNet', color=COLOR_UNET, alpha=0.85, zorder=3)
    ax1.bar(x, nnunet_dice, width, yerr=nnunet_err, capsize=4, label='3D nnU-Net Baseline', color=COLOR_NNUNET, alpha=0.9, zorder=3)
    ax1.bar(x + width, visreg_dice, width, yerr=visreg_err, capsize=4, label='3D VisReg JEPA (FPN)', color=COLOR_VISREG, alpha=0.9, zorder=3)
    ...
    ax2.bar(x - width, unet_hd, width, yerr=unet_hd_err, capsize=4, color=COLOR_UNET, alpha=0.85, zorder=3)
    ax2.bar(x, nnunet_hd, width, yerr=nnunet_hd_err, capsize=4, color=COLOR_NNUNET, alpha=0.9, zorder=3)
    ax2.bar(x + width, visreg_hd, width, yerr=visreg_hd_err, capsize=4, color=COLOR_VISREG, alpha=0.9, zorder=3)
...
def plot_missing_modality():
    scenarios = ['Clean 4-Sequence\n(All Modalities)', 'Missing Sequence: T1c Only\n(Contrast-Enhanced)', 'Missing Sequence: FLAIR Only\n(Edema-Weighted)']
    ...
    b1 = ax.bar(x - width, unet_dice, width, yerr=unet_err, capsize=4, label='3D Residual UNet', color=COLOR_UNET, alpha=0.85, zorder=3)
    b2 = ax.bar(x, nnunet_dice, width, yerr=nnunet_err, capsize=4, label='3D nnU-Net Baseline', color=COLOR_NNUNET, alpha=0.9, zorder=3)
    b3 = ax.bar(x + width, visreg_dice, width, yerr=visreg_err, capsize=4, label='3D VisReg JEPA (FPN)', color=COLOR_VISREG, alpha=0.9, zorder=3)
    
    # Annotate missing sequence resilience
    ax.annotate(r'$\mathbf{+12.1\%}$ over nnU-Net' + '\n' + r'($76.2\%$ vs. $64.1\%$ Dice)', 
                xy=(1 + width, 76.20), xytext=(1 + width - 0.1, 84.0),
                arrowprops=dict(facecolor=COLOR_VISREG, shrink=0.08, width=1.5, headwidth=6),
                fontsize=9.5, fontweight='bold', color=COLOR_VISREG, ha='center',
                bbox=dict(boxstyle='round,pad=0.3', facecolor='#fff0f0', edgecolor=COLOR_VISREG, alpha=0.9))
