...
\end{enumerate}
In combined segmentation objectives (Dice + Cross-Entropy), when background is excluded ($\text{include\_background}=\text{False}$), Cross-Entropy identically masks out class 0 ($\text{ignore\_index}=0$), guaranteeing mutually consistent foreground-only supervision without multi-task gradient conflict.
